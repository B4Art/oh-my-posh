...
Describe "Get-Provider" {
    It "uses the provider 'AwesomeSauce'" {
        ...
        $path = New-MockPath -Location 'C:\Users\bvandendonk\Test' -ProviderName $expected
...
Describe "Get-FormattedRootLocation" {
    Context "Running in the FileSystem" {
        BeforeAll { 
            Mock Get-Home { return 'C:\Users\bvandendonk' } 
            Mock Test-Windows { return $true }
        }
        It "is in the $HOME folder" {
            $path = New-MockPath -Location 'C:\Users\bvandendonk' -ProviderName 'FileSystem' -DriveName 'C'
            Get-FormattedRootLocation $path | Should -Be $ThemeSettings.PromptSymbols.HomeSymbol
        }
        It "is somewhere in the $HOME folder" {
            $path = New-MockPath -Location 'C:\Users\bvandendonk\Git\Somewhere' -ProviderName 'FileSystem' -DriveName 'C'
            Get-FormattedRootLocation $path | Should -Be $ThemeSettings.PromptSymbols.HomeSymbol
        }
...
Describe "Get-FullPath" {
    Context "Running in the FileSystem" {
        BeforeAll { Mock Get-Home { return 'C:\Users\bvandendonk' } }
        It "is in the $HOME folder" {
            $path = New-MockPath -Location 'C:\Users\bvandendonk' -ProviderName 'FileSystem' -DriveName 'C'
